...
GainP, relGain = [], np.ones([spwNum, 2, UseAntNum])
...
#-------- Flux Density
ScanFlux, ScanSlope, ErrFlux, ScanEL, centerFreqList = np.zeros([scanNum, spwNum]), np.zeros([scanNum, spwNum]), np.zeros([scanNum, spwNum]), np.zeros([scanNum]), []
for spw_index in range(spwNum):
...
for scan_index in range(scanNum):
    ...
    text_sd = ' %02d %016s   %4.1f' % (onsourceScans[scan_index], sourceList[sourceIDscan[scan_index]], 180.0* ScanEL[scan_index]/pi )
    ...
    for spw_index in range(spwNum):
        ...
        StokesI_PL = figScan.add_subplot( 2, spwNum/2, spw_index + 1 )
