...
for band_index in range(NumBands):
    ...
    spwNum = len(ambSpec) / antNum / scanNum
    #-------- Time-independent Trx spectrum
    print '---Analyzing ' + UniqBands[band_index] + ' Time-independent Trec using atmCal scans'
    tempAmb, tempHot  = np.zeros([useAntNum]), np.zeros([useAntNum])
    for ant_index in range(useAntNum):
        ...
        for spw_index in range(spwNum):
            chNum = ambSpec[spw_index* atmscanNum].shape[1]
            chRange = range(int(0.05*chNum), int(0.95*chNum)); chOut = sort(list(set(range(chNum)) - set(chRange))).tolist()
            TrxSpec = np.zeros([2, chNum, atmscanNum])
            for pol_index in range(2):
                for scan_index in range(atmscanNum):
                    index = (ant_index* spwNum + spw_index)* atmscanNum + scan_index
                    Psamb, Pshot = ambSpec[index][pol_index], hotSpec[index][pol_index]
