...
#
TrxList, TskyList = [], []
TsysSpec, chAvgTsys = [], []   # TsysSpec is a list [band, scan, ant, spw, pol] and each element is np.arryy[ch]
for band_index in range(NumBands):
    ...
    for ant_index in range(antNum):
        ...
        for spw_index in range(spwNum):
            ...
            Trx, Tsky = np.zeros([2, chNum, atmscanNum]), np.zeros([2, chNum, atmscanNum])
            for pol_index in range(2):
                for scan_index in range(atmscanNum):
                    ...
                    Trx[pol_index, :, scan_index] = (tempHot[ant_index]* Psamb - Pshot* tempAmb[ant_index]) / (Pshot - Psamb)
                    Tsky[pol_index, :, scan_index]= (Psoff* (tempHot[ant_index] - tempAmb[ant_index]) + tempAmb[ant_index]* Pshot - tempHot[ant_index]* Psamb) / (Pshot - Psamb)
                    Trx[pol_index, chOut, scan_index] = np.median(Trx[pol_index, chRange, scan_index])
                    Tsky[pol_index, chOut, scan_index] = np.median(Tsky[pol_index, chRange, scan_index])
            ...
            #
            TrxList = TrxList + [np.median(Trx, axis=2)] # Trx is time invariable
            TskyList= TskyList+ [np.mean(Tsky, axis=0)]  # Sky is unpolarized
        #
    #
    ...
    for spw_index in range(spwNum):
        ...
        for ant_index in range(antNum):
            ...
            #
        #
        Tau0spec = Tau0spec + [np.median(chTau, axis=0)]    
        ...
        for ant_index in range(antNum):
            for spw_index in range(bpspwNum):
                ...
                for pol_index in range(2):
                    ...
                    TsysSpec = TsysSpec + [Tsys]
                    chAvgTsys = chAvgTsys + [np.mean(Tsys)]
                #
            #
        #
    #
    ...
    for ant_index in range(antNum):
        ...
        for spw_index in range(spwNum):
            for pol_index in range(2):
                index = ((((band_index* scanNum + arange(scanNum))* antNum + ant_index)* spwNum + spw_index)* 2 + pol_index).tolist()
