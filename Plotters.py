...
#
#-------- Plot optical depth
def plotTau(prefix, antList, spw, secZ, tempAmb, Tau0med, TantN, TrxFlag, plotMax, PLOTFMT='png'):
    PolList = ['X', 'Y']
    ...
    antNum, spwNum  = len(antList), len(spw)
    for spw_index in range(spwNum):
        for pol_index in range(2):
            TskyPL = figTau.add_subplot(2, spwNum, spwNum* pol_index + spw_index + 1 )
            TskyPL.axis([1.0, 1.25*np.max(secZ), 0.0, plotMax])
            TskyPL.plot( airmass, 2.713* np.exp(-Tau0med[spw_index]* airmass) + (tempAmb - 20.0)* (1.0 - np.exp(-Tau0med[spw_index]* airmass)), '-')
    ...
    return
#
def plotTsys(prefix, antList, ambTime, spw, TrxList, TskyList, timeThresh=30.0, PLOTFMT='png'):
