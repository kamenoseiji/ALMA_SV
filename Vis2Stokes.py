...
solution = np.array([0.1, 0.1, np.angle( np.mean(Vis[1])), 0.0, 0.0])
...
for index in range(10):
    ...
    visvec = np.r_[ Vis[1].real, Vis[1].imag ]
...
text_sd = 'Q/I = %6.3f   U/I = %6.3f   XY_phase = %6.3f rad' % (solution[0], solution[1], solution[2]); plt.text(min(AzEl[3]), min(Vis[1].real), text_sd, size='x-small')
