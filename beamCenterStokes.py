...
for file_index in range(1):
    ...
    #BPantList = np.load(wd + prefix[file_index] + '.Ant.npy') 
    ...
    if refantName not in antList[trkAnt]:
        ...
        sys.exit()
    #
    refantID = np.where( antList == refantName)[0][0]; trkAnt.remove(refantID)
    antMap = [refantID] + trkAnt
    #BPantMap = indexList(antList[antMap], BPantList)
    #-------- AZ, EL, PA
    azelTime, AntID, AZ, EL = GetAzEl(msfile)
    azelTime_index = np.where( AntID == refantID )[0].tolist()
    #-------- Time index at on axis
    #matchNum = np.zeros([timeNum])
    #for time_index in range(timeNum): matchNum[time_index] = timeMatch(timeStamp[time_index], azelTime[azelTime_index], np.median(interval))
    #
    Az, El, PA = np.ones([timeNum]), np.ones([timeNum]), np.ones([timeNum])
    for time_index in range(timeNum):
        Az[time_index], El[time_index] = AzElMatch(timeStamp[time_index], azelTime[azelTime_index], np.median(interval), AZ, EL)
    #
    ...
    caledVis = np.mean(chAvgVis / (Gain[polYindex][:,ant0]* Gain[polXindex][:,ant1].conjugate()), axis=1)
#
"""
    #-------- Time Range
