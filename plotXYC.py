...
XYV = 0.5*(XYV[0] + XYV[1].conjugate())
XYC = 0.5*(XYC[0] + XYC[1].conjugate())
#-------- Plots
...
DTPL.plot(DT, np.arctan(XYV.imag / XYV.real)*180.0/pi, '.', label='Raw XY correlation')
CTPL.grid()
CTPL.plot(DT, np.arctan(XYC.imag / XYC.real)*180.0/pi, '.', label='residual after correction')
